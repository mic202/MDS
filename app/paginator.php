...
class Paginator {
    ...
    //PRINT LINKS
    public function createLinks( $links, $list_class, $url=null ) 
    {
        ...
        $showEnd = (($this->_row_start)+($this->_limit));
        if($showEnd > $this->_total)
            $showEnd = $this->_total;
/**        
        //debugging
...
**/
        $html = '<table border=0 width="100%">
                    <tr>
                        <td>
                            Showing '. (($this->_row_start)+1) .' to '. $showEnd . ' of '. $this->_total .' entries
                        </td>
                        <td align="right">';
        ...
        '<li class="' . $class . '"><a href="?limit=' . $this->_limit . '&page=' . ( $this->_page - 1 ) . $url .'">Previous</a></li>';

        $html .= $previous_page;

        if ( $start > 1 ) { //print ... before (previous <<< link)
            $html .= '<li><a href="?limit=' . $this->_limit . '&page=1'. $url .'">1</a></li>'; //print first page link
        ...
        for ( $i = $start ; $i <= $end; $i++ ) {
            ...
            $html .= '<li class="' . $class . '"><a href="?limit=' . $this->_limit . '&page=' . $i . $url .'">' . $i . '</a></li>';
        }

        if ( $end < $last ) { //print ... before next page (>>> link)
            $html .= '<li class="disabled"><span>...</span></li>'; //print 3 dots if not on last page
            $html .= '<li><a href="?limit=' . $this->_limit . '&page=' . $last . $url .'">' . $last . '</a></li>'; //print last page link
        }
        ...
        '<li class="' . $class . '"><a href="?limit=' . $this->_limit . '&page=' . ( $this->_page + 1 ) . $url .'">Next</a></li>';
